...
当前版本：`Controller & Analysers v16.22.260701.0042`
...
Baseline 可以使用同一组拟合参数校正一个或多个单光谱/多光谱 TXT/WDF 文件。

1. 使用 `Load txt/wdf` 或 `Add` 把文件加入列表。Ctrl/Shift 多选、Delete 和拖动排序与 Converting 列表相同。
2. 勾选的文件会一起显示在 Preview 中；如果同时选中多行，更改其中一行的勾选状态会应用到所有选中行。
3. 对于多光谱文件，填写一个共用的 `Selected columns` 并点击 `Update`；每个勾选的多光谱文件都会预览相同的 1-based 列号。拟合仍会处理全部光谱。
4. 选择 `asPLS`、`drPLS` 或 `Polynomial/backcor`，然后使用 Auto 模式，或在 Manual 模式点击 `Fit`。Fit 会用同一组参数处理列表中的全部文件。
5. 只有列表中恰好有一个文件时，`Load fitted`、Output filename 和 `Save` 才保持可用。
6. `Save all` 选择一个文件夹，把所有成功拟合的条目保存为 `<原文件名>_Copy.txt`，并保留各自原始的逻辑 TXT 格式。

`Preview` 标签页上方叠加勾选文件的原始光谱/基线，下方叠加校正后的光谱。
